...
# Start the prompt, appending a powerline segment
prompt_start() {
  if [[ -n $CURRENT_BG ]]; then
    echo -n " %{%k%F{$CURRENT_BG}%}$SEGMENT_SEPARATOR_START"
  else
    echo -n "%{%k%}"
  fi
  echo -n "%{%f%}"
...
prompt_dir() {
  prompt_segment black default $CURRENT_FG '%~'
}
